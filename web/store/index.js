...
export const getters = {
  isSuperuser: state => state.type === 'superuser',
  
  getDaycareId: state => state.auth.user.daycare_id,

  weeks: () => Array.from(Array(54).keys())
    .filter(item => item > 0)
    .map(item => ({
        text: `Week ${item}`,
        value: item 
...
export const actions = {
  ...
  async updateProfile({ state, commit, dispatch }, payload) {
    try {
      ...
      const response = await this.$axios.$post(`/api/auth/update`, state.auth.user)
  ...
  async updatePassword({ commit }, payload) {
    try {
      await this.$helpers.loader()
      const response = await this.$axios.$put(`/api/auth/update-password`, payload.credentials)
